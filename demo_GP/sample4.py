...
func_4d_case_3 = func_4D(round_4d=None)
round_xy = func_4d_case_3.round_x

# 测试数据
test_point_mun = 1000
x_test = A_gpr.sample_point(func_4d_case_3.round_x, iter=test_point_mun)
y_test = [func_4d_case_3.f_obj(x_test[i, 0], x_test[i, 1], x_test[i, 2], x_test[i, 3]) for i in range(x_test.shape[0])]

lf_point_num = 10
# l-f 数据采集
x_4D_l = A_gpr.sample_point(round_xy=round_xy, iter=lf_point_num)
y_4D_l = np.array([func_4d_case_3.f_l(x_4D_l[i, 0], x_4D_l[i, 1], x_4D_l[i, 3], x_4D_l[i, 3]) for i in
...
for it in np.array(range(left, right, dist)):
    ...
    for i in range(10):
        ...
        y_init = np.array([func_4d_case_3.f_obj(x_init[i, 0], x_init[i, 1], x_init[i, 2], x_init[i, 3]) for i in range(x_init.shape[0])]).reshape(-1, 1)

        hf_gp, list_w_hf = A_gpr.creat_gp_model(max_loop=temp_it, func_nd=func_4d_case_3, x_init_l=x_4D_l, y_init_l=y_4D_l,
                                                x_init_h=x_init, y_init_h=y_init, round_x=round_xy,
                                                n_start=1, n_single=200, min_dist=0.02)
        ...
        # 对比试验
        x_sample_point = A_gpr.sample_point(func_4d_case_3.round_x, iter=it)
        y_sample_point = [func_4d_case_3.f_obj(x_sample_point[i, 0], x_sample_point[i, 1], x_sample_point[i, 2], x_sample_point[i, 3]) for i in range(x_sample_point.shape[0])]
